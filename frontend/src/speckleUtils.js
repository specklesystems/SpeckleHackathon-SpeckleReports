...
export function goToSpeckleAuthPage() {
  ...
  window.location = `${process.env.VUE_APP_SERVER_URL}/authn/verify/${process.env.VUE_APP_SPECKLE_ID}/${challenge}`
}
...
export const getStreamBranches = (streamId, itemsPerPage, cursor) => speckleFetch(streamBranchesQuery, {streamId, limit: itemsPerPage, cursor}).then(res => res.data?.stream)

export const createWebhook = (streamId, url, triggers) => speckleFetch(`mutation($webhook: WebhookCreateInput!) {
    webhookCreate(webhook: $webhook)
}`, { webhook: { streamId, url, triggers }})